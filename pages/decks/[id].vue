<template>
  <div>
    ...
    <div v-else class="mt-8">
      ...
      <!-- Cards in the deck -->
      <div v-if="deck.cards && deck.cards.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="card in deck.cards" :key="card.id" class="bg-white shadow rounded p-4 flex justify-between">
          <div>
...
const availableCards = computed(() => {
  ...
  const deckCardIds = deck.value.cards.map(card => card.id);
...
const addCardToDeck = async (cardId) => {
  ...
  await deckStore.addCardToDeck(deckId.value, cardId);
};
...
const removeCardFromDeck = async (cardId) => {
  ...
  if (confirm('Are you sure you want to remove this card from the deck?')) {
    await deckStore.removeCardFromDeck(deckId.value, cardId);
  }
};
